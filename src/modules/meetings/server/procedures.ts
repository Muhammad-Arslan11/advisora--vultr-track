...
const transcript = await fetch(existingMeeting.transcriptUrl).then((res) => res.text()).then((text) => JSONL.parse<StreamTranscriptItem>(text)).catch(() => {
  return [];
});
...
const userSpeakers = await db.select().from(user).where(inArray(user.id, speakerIds)).then((users) => users.map((user) => ({
  ...user,
  image: 
  user.image ?? GeneratdAvatarUri({seed: user.name, variant:"initials"}),
}))
    ...
    await streamVideo.upsertUsers([
      {
        ...
        image:
          ctx.auth.user.image ??
          GeneratdAvatarUri({ seed: ctx.auth.user.name, variant: "initials" }),
      },
    ]);
  ...
  create: protectedProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      if (!existingAgent) {
        throw new TRPCError({
          code: "NOT_FOUND",
          message: "Agent not found, assign one to this meeting and try again",
        });
      }
      await streamVideo.upsertUsers([
        {
